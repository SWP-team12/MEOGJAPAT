...
import kotlinx.android.synthetic.main.fragment_prt_list.*
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

class PrtListFragment : Fragment(), Refreshable {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // RecyclerView
        rv_prt_meeting_list.layoutManager = LinearLayoutManager(activity)
        ...
        rv_prt_meeting_list.adapter = meetingAdapter
    }
    ...
    override fun refresh() {
        ...
        val readMeetingListCall = GlobalApplication.INSTANCE.api.readMeetingList(0, GlobalApplication.INSTANCE.id.toInt())
        readMeetingListCall.enqueue(object : Callback<List<Meeting>> {
            override fun onResponse(call: Call<List<Meeting>>, response: Response<List<Meeting>>) {
                if (response.isSuccessful) {
                    ...
                    Log.d("Prt", "Server Error - ${response.message()}")
